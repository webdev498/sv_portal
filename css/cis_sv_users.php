			...
			<!-- tabs start -->
				...
				if (!$kazooAccountID) {
					...
					$sql = "select u.*, " .
							...
							"(select count(0) from KazooStatusEvents where type IN ('INBOUND ERROR','OUTBOUND ERROR') and eventDate > date_sub(current_date, INTERVAL 1 DAY) " .  
								"and userId IN (select userId from KazooUsers  where last_name=u.last_name and accountId='{$kazooAccountID}')) as cdrEvents, " .
							...
							"from KazooUsers u where accountId='{$kazooAccountID}' and first_name not like 'X_%' group by u.last_name order by downDevices DESC, cdrEvents DESC, u.last_name, u.first_name";
					...
					else
					{
						while($row = mysql_fetch_array($retval, MYSQL_ASSOC))
						{
							...
							if ($cdrEvents>0) {
								...
								$cdrIcon = "<a data-toggle='tooltip' title='{$cdrEvents} recent call events.'><i class='fa fa-exclamation text-danger'></i></a>";
							}
