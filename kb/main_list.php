...
if ($useAJAX) {
	$includes.="var AUTOCOMPLETE_TABLE='main_autocomplete.php';\r\n";
	$includes.="var INLINE_EDIT_TABLE='main_edit.php';\r\n";
	$includes.="var INLINE_ADD_TABLE='main_add.php';\r\n";
	$includes.="var INLINE_VIEW_TABLE='main_view.php';\r\n";
	$includes.="var SUGGEST_TABLE='main_searchsuggest.php';\r\n";
	$includes.="var MASTER_PREVIEW_TABLE='main_masterpreview.php';\r\n";
	$includes.="var SUGGEST_LOOKUP_TABLE='main_lookupsuggest.php';";
}
$includes.="\n</script>\n";
if ($useAJAX) {
...
//			$strSQL = AddWhere($strSQL,"1=0");
	...
	{
		$strWhere = "1=0";
		if($where=StrWhere("approved", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
...
$smarty->assign("pagination","<script language=\"JavaScript\">WritePagination(".$mypage.",".$maxpages.");
		...
		{
			window.location='main_list.php?goto='+nPageNumber;
		}
</script>");
...
//	fill $rowinfo array
	...
	while($data && $recno<=$PageSize)
	{
		...
		for($col=1;$data && $recno<=$PageSize && $col<=1;$col++)
		{
			...
			$row[$col."editable"]=CheckSecurity($data["Category"],"Edit");
//	key fields
			$keyblock="";
...
//	fill in search variables
	//	field selection
		if(@$_SESSION[$strTableName."_searchfield"]=="approved")
			$smarty->assign("search_approved","selected");
