...
if ($useAJAX) {
	$includes.="var AUTOCOMPLETE_TABLE='_register_autocomplete.php';\r\n";
	$includes.="var INLINE_EDIT_TABLE='_register_edit.php';\r\n";
	$includes.="var INLINE_ADD_TABLE='_register_add.php';\r\n";
	$includes.="var INLINE_VIEW_TABLE='_register_view.php';\r\n";
	$includes.="var SUGGEST_TABLE='_register_searchsuggest.php';\r\n";
	$includes.="var MASTER_PREVIEW_TABLE='_register_masterpreview.php';\r\n";
	$includes.="var SUGGEST_LOOKUP_TABLE='_register_lookupsuggest.php';";
}
$includes.="\n</script>\n";
if ($useAJAX) {
...
$smarty->assign("order_dir_fullname","a");
...
if(@$_SESSION[$strTableName."_orderby"])
{
	...
	$smarty->assign("order_dir_fullname","a");
	if($order_field=="fullname")
	{
		if($order_dir=="a")
		{
			$smarty->assign("order_dir_fullname","d");
			$img="up";
		}
		else
			$img="down";
		$smarty->assign("order_image_fullname","<img src=\"images/".$img.".gif\" border=0>");
	}
	$smarty->assign("order_dir_username","a");
...
if (@$_REQUEST["mdelete"])
{
	...
	{
		$keys=array();
		$keys["email"]=refine($_REQUEST["mdelete1"][$ind-1]);
		$selected_recs[]=$keys;
	}
}
...
$smarty->assign("pagination","<script language=\"JavaScript\">WritePagination(".$mypage.",".$maxpages.");
		...
		{
			window.location='_register_list.php?goto='+nPageNumber;
		}
</script>");
...
//	key fields
			$keyblock="";
			$row[$col."id1"]=htmlspecialchars($data["email"]);
			$keyblock.= rawurlencode($data["email"]);
			$row[$col."keyblock"]=htmlspecialchars($keyblock);
			$row[$col."recno"] = $recno;
//	detail tables
//	edit page link
			$editlink="";
			$editlink.="editid1=".htmlspecialchars(rawurlencode($data["email"]));
			$row[$col."editlink"]=$editlink;

			$copylink="";
			$copylink.="copyid1=".htmlspecialchars(rawurlencode($data["email"]));
...
//	fullname - 
			$value="";
				$value = ProcessLargeText(GetData($data,"fullname", ""),"field=fullname".$keylink,"",MODE_LIST);
			$row[$col."fullname_value"]=$value;
...
$templatefile = "_register_list.htm";
